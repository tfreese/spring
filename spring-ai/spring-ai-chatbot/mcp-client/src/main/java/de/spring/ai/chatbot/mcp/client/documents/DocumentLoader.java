...
public final class DocumentLoader {
    ...
    /**
     * Already done in {@link TikaDocumentReader} by {@link ExtractedTextFormatter}.
     */
    private static Document cleanupDocument(final Document document) {
        ...
        final String text = Optional.ofNullable(document.getText())
                ...
                // .map(ExtractedTextFormatter::trimAdjacentBlankLines)
    ...
    private static Stream<Document> readDocumentsFromResource(final Resource resource, final ExtractedTextFormatter extractedTextFormatter) {
    ...
    List<Document> loadDocuments(final ChatModel chatModel, final List<String> locationPatterns) {
        ...
        final List<Document> documents = resources.stream()
                ...
                .flatMap(resource -> readDocumentsFromResource(resource, extractedTextFormatter))
